class GameModeScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(width/2, 250, 'MODO DE JUEGO', { fontSize: '64px', fill: '#fff' }).setOrigin(0.5);

        const localButton = this.add.text(width/2, 450, 'LOCAL', { fontSize: '32px', fill: '#fff' })
            .setOrigin(0.5)
            .setInteractive();
        ...
        const redButton = this.add.text(width/2, 650, 'RED', { fontSize: '32px', fill: '#858585' })
            .setOrigin(0.5);
